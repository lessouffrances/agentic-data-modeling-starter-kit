...
- No build artifacts, compiled models, or `node_modules` staged

If violations are found, list them and stop. Do not commit.

## Step 3 — Run quality checks
Run the project's lint, type-check, and (when configured) data test commands from the tooling section of `BOOTSTRAP_FORM.md`.
...
Examples:
- `feat(models): add orders_fct and customer_dim`
- `fix(pipelines): handle late-arriving events for sessions`
- `test(quality): add uniqueness tests for orders_fct`
